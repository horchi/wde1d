   ...
   public:
      ...
      static int init()
      {
         if (mysql_library_init(0, 0, 0)) 
            return errorSql(0, "init(library_init)");
      ...
      { 
         inTact = yes;
         return mysql_query(getMySql(), "START TRANSACTION"); 
      }

      virtual int commit()
      { 
         inTact = no;
         return mysql_query(getMySql(), "COMMIT"); 
      }

      virtual int rollback() 
      { 
         inTact = no;
         return mysql_query(getMySql(), "ROLLBACK"); 
      }
